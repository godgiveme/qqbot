...
// =========================================================================
// [核心修复] 将全局状态改为 Map，按 appId 隔离，解决多账号串号问题
// =========================================================================
...
async function doFetchToken(appId: string, clientSecret: string): Promise<string> {
  ...
  console.log(`[qqbot-api:${appId}] <<< Headers:`, JSON.stringify(responseHeaders, null, 2));
...
/**
 * 停止后台 Token 刷新
 * @param appId 选填。如果有，仅停止该账号的定时刷新；如果没有，停止所有。
 */
...
/**
 * 检查后台 Token 刷新是否正在运行
 * @param appId 选填。如果有，检查特定账号；如果没有，检查是否有任意账号正在运行。
 */
